...
class Base {
	...
	public function recovery( $config, \Closure $callback = null ) {
		...
		if ( is_file( $config['dir'] . '/_recovery.php' ) ) {
			...
			foreach ( Dir::tree( $config['dir'] ) as $f ) {
				//不运行执行的文件
				if ( in_array( $f['basename'], [ '_config.php', '_structure.php', '_lock.php' ] ) ) {
					continue;
				} else {
					$file[] = $f['path'];
				}
			...
			return $callback( [ 'message' => '表结构还原成功,准备开始还原数据', 'status' => 'run' ] );
		}
	}
	...
	protected function recoveryRun( \Closure $callback ) {
		...
		foreach ( $config['file'] as $id => $f ) {
			...
			//完成比例
			$bl = ( intval( ( $config['totalfile'] - count( $config['file'] ) ) / $config['totalfile'] * 100 ) );
			$this->saveConfig( '_recovery.php' );
	...
	public function backup( array $config, \Closure $callback = null ) {
		//备份文件夹
		$dir           = isset( $_COOKIE['houdunwang_backup_dir'] ) ? urldecode( $_COOKIE['houdunwang_backup_dir'] ) : $config['dir'];
		$config['dir'] = $dir;
		...
		if ( is_file( $dir . '/_config.php' ) ) {
			...
			setcookie( 'houdunwang_backup_dir', urlencode( $dir ) );
	...
	protected function backupRun( \Closure $callback ) {
		...
		foreach ( $cache['table'] as $table => $config ) {
			...
			do {
				...
				if ( empty( $data ) ) {
					...
					foreach ( $data as $d ) {
						$sql .= "\houdunwang\db\Db::table('{$table}',true)->replace(" . var_export( $d, true ) . ");\n";
					}
				}
				//检测本次备份是否超出分卷大小
		...
		setcookie( 'houdunwang_backup_dir', 0, 1 );
	...
	protected function saveConfig( $file ) {
		return file_put_contents( $this->config['config']['dir'] . '/' . $file, '<?php return ' . var_export( $this->config, true ) . ';' );
	}
